...
- [ ] At-Risk Engine: **rule-based thresholds only for this phase** (e.g. 3+ skill gaps AND low mock score AND low activity). Do not build a trained classifier yet — if upgraded later, class imbalance must be handled via SMOTE or class weighting first, per `ARCHITECTURE.md` §5, or the model will silently predict "not at risk" for nearly everyone
...
- [ ] Write an equivalent evaluation statement for the Readiness Score and At-Risk flags — a **face-validity review**: manually inspect ~10 seeded student profiles, confirm the readiness band and any risk flag each receives matches what a human would reasonably expect given their inputs, and note any mismatches. This satisfies the official deliverable "accuracy/performance evaluation of matching and scoring" for the scoring side, not just matching — per `ARCHITECTURE.md` §9, do not report a fabricated accuracy percentage here either

**Definition of done:** The full lifecycle — Profiling → Matching → Scheduling → Notification → Offer Tracking → Analytics — works end-to-end on the live deployment, and both evaluation statements (Matching Engine sanity check + Readiness/At-Risk face-validity review) are written down, not just implied.
