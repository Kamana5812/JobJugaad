...
def predict_signal(inputs: PlacementModelInput):
    ...
    import numpy as np
    import pandas as pd
    frame = pd.DataFrame([values], columns=_report["feature_columns"])
    preprocessing = _pipeline.named_steps["preprocess"]
    forest = _pipeline.named_steps["classifier"]
    ...
    numeric = _report["protocol"]["numeric_features"]
    categorical = _report["protocol"]["categorical_features"]
    ...
    score = float(_pipeline.predict_proba(frame)[0, 1] * 100)
