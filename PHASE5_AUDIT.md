...
## BTech extension — current policy inventory, 2026-09-24

Application release bb3f743/API 0.8.0 adds btech_model_profiles, bringing the total to **19 tenant tables**. The new table has college_id, ENABLE/FORCE college_isolation with read/write predicates, application college filters, student ownership and a composite student/college foreign key. The live report verifies every table and the non-bypass role: [19-table evidence](evaluations/btech-live-security.json). The BTech screen returns a baseline, four factors and explanation with every score, and no score when inputs are missing/outside source coverage. All 51 backend tests, production build, actual-output rendering and live API/persistence/access checks passed. No interactive browser test is claimed because automation cannot initialize.
