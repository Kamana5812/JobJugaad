...
function Layout() {
  ...
  return <div className="min-h-screen bg-paper font-sans leading-relaxed text-ink">
    ...
    <header className="border-b border-line bg-white"><nav aria-label="Main navigation" className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-6">
      ...
      <div className="flex items-center gap-4 text-sm font-semibold text-navy">{user ? <><Link to="/student/profile">My profile</Link><button onClick={logout} className={secondaryStyle}>Log out</button></>
        : <><Link to="/login">Log in</Link><Link to="/signup" className={secondaryStyle}>Get started</Link></>}</div>
    </nav></header>
    <main id="main" className="mx-auto max-w-6xl px-5 py-8 sm:px-6 sm:py-12">
      {loading ? <p role="status">Restoring your session…</p> : <Routes>
        ...
        <Route path="/student/profile" element={user ? <ProfilePage /> : <Navigate to="/login" replace />} />
    ...
    <footer className="border-t border-line px-6 py-6 text-center text-xs text-muted">JobJugaad · Explainability-first CampusLink prototype · Student Core</footer>
  </div>
}
