...
- **If an At-Risk classifier goes beyond P0 rule-based thresholds, class imbalance must be handled** (SMOTE or class weighting) before it ships. At-risk students are always a minority class; an untreated classifier will silently predict "not at risk" for nearly everyone while looking falsely accurate. See `ARCHITECTURE.md` §5, Predictive AI.
